...
const bodySchema = z.object({ gameQuestionId: z.string().min(1).max(64), answerId: z.string().min(1).max(64) });
...
export async function POST(request: NextRequest, ctx: RouteContext<"/api/games/[id]/answer">) {
  ...
  const rl = rateLimit(`answer:${player.id}`, 20, 10_000);
  ...
  try {
    const result = await submitAnswer({ gameId, playerId: player.id, ...parsed.data });
    return Response.json(result, { headers: { "Cache-Control": "no-store" } });
  } catch (err) {
    if (err instanceof EngineError) {
      const status = err.code === "DUPLICATE" ? 409 : err.code === "CLOSED" ? 410 : err.code === "FORBIDDEN" ? 403 : 400;
      return Response.json({ error: err.message, code: err.code }, { status });
    }
    console.error("[answer] unexpected", err);
